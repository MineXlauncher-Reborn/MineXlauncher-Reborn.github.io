...
function consoleLog(type, msg) {
    ...
    if (consoleElement) {
        const scrolledToBottom = Math.abs(consoleElement.scrollHeight -
            consoleElement.scrollTop -
            consoleElement.clientHeight) === 0;
...
const game = {
    play: function (version) {
        if (window !== window.top)
            ...
            if (!window.gameWindow || window.gameWindow.closed) {
                ...
                window.gameWindow = window.open(noPopup ? version : 'about:blank', '_blank', `popup=${noPopup ? 'false' : 'true'}`);
                if (window.gameWindow) {
                    if (!noPopup) {
                        ...
                        ['keydown', 'keyup'].forEach((eventType) => window.gameWindow?.addEventListener(eventType, (event) => iframe.contentWindow?.dispatchEvent(new KeyboardEvent(eventType, event))));
                        window.gameWindowIframe = iframe.contentWindow;
                    }
                    else
                        window.gameWindowIframe = window.gameWindow;
                    if (window.gameWindowIframe) {
                        ['debug', 'log', 'info', 'warn', 'error'].forEach((type) => {
                            if (window.gameWindowIframe)
                                window.gameWindowIframe.console[type] = (msg) => consoleLog(type, msg);
                        });
                    }
            ...
            const waitForCrash = setInterval(() => {
                if (window.gameWindow?.closed) {
                    ...
                    window.gameWindowIframe?.document
                        ...
                        .forEach((element) => {
                        if (element.innerText.includes("Game Crashed! I have fallen and I can't get up!")) {
                            clearInterval(waitForCrash);
    ...
    stop: function (error, options) {
        if (window !== window.top)
            window.top?.game.stop(error);
        else {
            if (window.gameWindow &&
...
const article = {
    open: function (articleId) {
        const modal = document.querySelector(`.article[data-article-id='${articleId}']`);
        const modalContent = document.querySelector(`.article[data-article-id='${articleId}'] > div`);
        if (!articleAnimationLock && modal && modalContent) {
            ...
            modal.addEventListener('animationend', () => {
                const closeArticleHandler = (event) => {
    ...
    session: {
        ...
        set: function (key, value) {
            ...
            if (item === null) {
                const json = {};
                json[key] = value;
                sessionStorage.setItem('minexlauncher-v2', base64Gzip.compress(JSON.stringify(json)));
            }
            else {
                const json = JSON.parse(base64Gzip.decompress(item));
                json[key] = value;
...
const addons = {
    mods: {
        toggle: function (modId) {
            ...
            const addons = storage.local.get('addons') ?? {
                mods: [],
            };
...
const backups = {
    export: async function () {
        ...
        for (const dbInfo of await indexedDB.databases()) {
            if (dbInfo.name)
                indexedDB.open(dbInfo.name).onsuccess = (event) => (exportData.indexedDb[dbInfo.name ?? ''] =
                    idbExportImport.exportToJsonString(event.target.result));
        }
        const url = URL.createObjectURL(new Blob([base64Gzip.compress(JSON.stringify(exportData))]));
    ...
    import: async function () {
        await new Promise((resolve, reject) => {
            ...
            if (typeof cookieStore !== 'undefined')
                ...
                document.cookie
                    .split('; ')
                    .forEach((cookie) => (document.cookie = `${cookie.split('=')[0]}=; Path=/; Max-Age=0`));
            localStorage.clear();
            for (const dbInfo of await indexedDB.databases())
                if (dbInfo.name)
            ...
            if (Object.keys(importData.indexedDb).length > 0)
                for (const dbName in importData.indexedDb) {
                    ...
                    dbRequest.onsuccess = async (event) => {
                        ...
                        transaction.oncomplete = () => idbExportImport.importFromJsonString(db, importData.indexedDb[dbName]);
                    };
                }
...
if (window.location.pathname === '/') {
    ...
    document.addEventListener('DOMContentLoaded', async () => {
        ...
        if (titleBarText)
            titleBarText.textContent += ` ${currentVersion}`;
        if (lastVersion &&
            gt(
            // @ts-expect-error
            coerce(currentVersion, { includePrerelease: true }), coerce(lastVersion, { includePrerelease: true }))) {
...
if (window.location.pathname === '/settings/general/') {
    document.addEventListener('DOMContentLoaded', async () => {
        ...
        const offlineCheckbox = document.querySelector('#offline-checkbox');
        const adsCheckbox = document.querySelector('#ads-checkbox');
        const themeData = (await (await fetch('/resources/data/main.json')).json()).themes;
        if (usernameInput) {
        ...
        if (noPopupCheckbox) {
            ...
            noPopupCheckbox.addEventListener('change', () => storage.local.set('noPopup', noPopupCheckbox.checked));
        }
        if (offlineCheckbox) {
        ...
        if (setupForm) {
            ...
            setupForm.addEventListener('submit', async (event) => {
                ...
                if (!username) {
                    ...
                    storage.local.set('lastVersion', (await (await fetch('/resources/data/main.json')).json()).updates[0]
                        .version);
                    if (offlineCheckbox?.checked) {
                        sw.register('/sw-full.js');
                        alert('Offline cache is now downloading.\nThe download size is about 1GB, so it may take a while.\n\nPlease do not leave this page while the download is in progress.\nYou will be notified when the download is complete.');
...
else if (window.location.pathname === '/addons/mods/' ||
    window.location.pathname === '/addons/resourcepacks/') {
    document.addEventListener('DOMContentLoaded', async () => {
        const addonType = window.location.pathname === '/addons/mods/' ? 'mods' : 'resourcepacks';
        const addons = (await (await fetch('/resources/data/main.json')).json()).addons;
        const addonList = document.querySelector('.mod-list');
        addons[addonType].forEach((addon) => {
            const addonItem = document.createElement('div');
            addonItem.innerHTML = `<img loading="lazy" src="/resources/images/icons/addons/${addon.id}.webp" /><div class="details"><strong>${addon.name}</strong><p class="author">By <a href="${addon.authorLink}" target="_blank">${addon.author}</a></p><p class="description">${addon.description}</p></div><div class="links">${addonType === 'mods'
                ...
                : `<span class="download" onclick="downloadFile('/resources/addons/${addon.id}.zip', '${addon.name.replace('\\', '\\\\').replace("'", "\\'")}.zip')">Download</span>`}</div>`;
    ...
    document.addEventListener('DOMContentLoaded', async () => {
        ...
        const data = (await (await fetch('/resources/data/main.json')).json()).updates;
        data.forEach((update) => {
...
else if (window.location.pathname === '/home/game/' ||
